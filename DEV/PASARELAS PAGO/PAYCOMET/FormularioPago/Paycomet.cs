...
namespace FormularioPago
{
    public class Paycomet
    {
            ...
            if (!String.IsNullOrEmpty(token) && token.Length == 64)
            {
                ...
                if (bs != null)
                {
                    ...
                    if (add_user_token.Result == "OK")
                    {
                        ...
                        string productDescription = new Auxiliares(ServicioID).SelectCampo("nombreServicio");// Descripción del producto
        ...
        public string getPaycometUrl()
        {
            ...
            if (bs != null)
            {
                //public BankstoreServResponse ExecutePurchaseUrl
                ...
                //string secure3D = "0",
                //string scoring = null)
                BankstoreServResponse execute_purchase = bs.ExecutePurchaseUrl(ServicioID, amount, "EUR", "ES", "", "1", "50");
                if (execute_purchase.Result == "OK")
                {
                    redirectUrl = execute_purchase.Data["URL_REDIRECT"];
                }
