...
namespace FlujoCompletoCompra.Recursos
{
    public class ErrorHandling
    {
        ...
        string ErrorTable;
        string ErrorOrigin;
        string ErrorLogPath = AppDomain.CurrentDomain.BaseDirectory + @"ErrorLogs\Log.txt";
        public static string emailErrores = ConfigurationManager.AppSettings["EmailErrores"];
        ...
        public ErrorHandling(string codigoError, string origin, string errorMsg = "")
        {
            string error = GetError(codigoError);
            ErrorOrigin = origin;
        ...
        /// </summary>
        /// <param name="codError">Codigo Error PayComet</param>
        /// <returns>Mensaje Error</returns>
        string GetError(string codError)
        {
            string res = "";
        ...
        /// <summary>
        /// Envia email Error a desarrollo
        /// </summary>
        void EnviarEmailError()
        {
            MailMessage mail = new MailMessage();
            SmtpClient smtp = new SmtpClient();
            mail.To.Add(new MailAddress(emailErrores));
            mail.Subject = "Error Gestrafico - " + pagina;
        ...
        /// <summary>
        /// Inserta Log Error en BBDD
        /// </summary>
        void InsertError()
        {
            ErrorQuery = "INSERT INTO [dbo].[LogErrores] ([Procedencia] ,[Mensaje]) VALUES ('" + ErrorOrigin.Replace("'", "") + "','" + ErrorMsg.Replace("'", "") + "')";
            try
            {
                SqlConnection connection = new SqlConnection(ConnectionString);
                SqlCommand command = new SqlCommand(ErrorQuery, connection);
                connection.Open();
        ...
        void WriteErrorLog()
        {
            ...
            try
            {
                string[] lines = new string[] { sb.AppendLine(dateTime + " - Error:" + ErrorMsg).ToString() };
                File.AppendAllText(ErrorLogPath, sb.AppendLine(dateTime + " - Error:" + ErrorMsg).ToString() 
                                                                                        + Environment.NewLine);
            }
            catch (Exception ex)
            {
